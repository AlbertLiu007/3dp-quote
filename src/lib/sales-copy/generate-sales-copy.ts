...
export function generateSalesCopy(input: {
  ...
  if (language === 'en') {
    return {
      shortMessage: `Hello, the quote for ${fileName} is ready: dimensions are about ${dimensions}. Recommended material/process: ${material.name}/${material.process.toUpperCase()}; ${materialHighlight} ${weightText}; ${priceText}; normal lead time is about ${material.leadTimeDays} days.`,
      detailedMessage: `Hello, the model has been parsed and quoted locally. Its dimensions are about ${dimensions}, with ${measurement.triangleCount.toLocaleString('en-US')} triangles. Recommended process: ${material.name}/${material.process.toUpperCase()}. ${materialHighlight} ${material.descriptionEn ?? material.description} ${weightText}; ${priceText}; normal lead time is about ${material.leadTimeDays} days. For higher strength, heat resistance, or finer surface quality, you can compare other material options.`,
  ...
  return {
    shortMessage: `您好，${fileName} 已完成报价：尺寸约 ${dimensions}，推荐 ${material.name}/${material.process.toUpperCase()}，${materialHighlight}，${weightText}，${priceText}，正常交期约 ${material.leadTimeDays} 天。`,
    detailedMessage: `您好，模型已完成本机解析和报价。该模型尺寸约 ${dimensions}，三角面片数 ${measurement.triangleCount.toLocaleString('zh-CN')}，建议使用 ${material.name}/${material.process.toUpperCase()} 工艺。${materialHighlight}。${material.description} ${weightText}，${priceText}，正常交期约 ${material.leadTimeDays} 天。如需更高强度、耐温或更细腻表面，也可以继续切换材料方案对比。`,
    riskNote: measurement.volumeCm3
